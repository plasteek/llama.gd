...
void LlamaWorker::input(std::string query)
{
   current_input = query;
   await_input = false;
}

bool LlamaWorker::is_waiting_input()
{
   return await_input;
}
...
std::string LlamaWorker::run(std::string prompt)
{
   ...
   if (!path_session.empty())
   {
      ...
      else
      {
         ...
         if (!llama_state_load_file(ctx, path_session.c_str(), session_tokens.data(), session_tokens.capacity(), &n_token_count_out))
         {
            ...
            throw std::runtime_error(std::string(__func__) + ": error: failed to load session file " + path_session);
         }
   ...
   while (!should_yield && ((n_remain != 0 && !is_antiprompt) || params.interactive))
   {
      // predict
      if (!embd.empty())
      {
      ...
      if ((int)embd_inp.size() <= n_consumed)
      {
         ...
         if (n_past > 0 && is_interacting)
         {
            ...
            // Here is the override for GODOT to make sure the user input
            await_input = true;
            on_wait_input();

            LOG("Waiting for user input\n");
            while (await_input && !should_yield)
            {
               std::this_thread::sleep_for(std::chrono::milliseconds(1));
            }
            LOG("Ending user input\n");
            std::string buffer = current_input;
